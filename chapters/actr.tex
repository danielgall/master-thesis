...
\label{actr:modular_organization}
...
\draw let
 ...
 node[module,  fill=red!20,  minimum width=\x4-\x3, text width=6em] at (\x1,\y2-7mm) (environment) {environment}; 
...
\tikzstyle{procmodule} = [rectangle, draw, fill=blue!20,  text centered, rounded corners, minimum height=3em]
\tikzstyle{buffer} = [rectangle, draw, fill=yellow!20, 
    text width=6em, text centered, rounded corners, minimum height=3em]
\tikzstyle{line} = [draw, -latex]
\tikzstyle{doubleline} = [draw, latex-latex]
...
& \coordinate (helper0) {};   \coordinate[right of=helper0] (helper1) {}; \\ 
\coordinate (helper2) {}; &
\node[procmodule, rectangle split, rectangle split parts=2, inner sep=2mm] (procmod) {
procedural module
\nodepart{second}
\begin{tikzpicture}[node distance=3.5cm]
\node[module] (procmem) {procedural memory};
\node[module, right of=procmem] (patmatch) {pattern matching};
\node[module, below of=patmatch, node distance=2cm] (prodexec) {production execution};
\end{tikzpicture}
};
&
\node[module] (declarative) {declarative module};
\\
&
\node[buffer] (buffers) {buffers};
\\
\node[module] (visual) {visual module};
&
\node[module,  fill=red!20] (environment) {environment}; 
&
\node[module] (manual) {manual module};
\\
};

\path (buffers.west) ++(0,-0.3) coordinate (bufsouthwest);
\path[line] (visual) |- (bufsouthwest);
\path (buffers.east) ++(0,-0.3) coordinate (bufsoutheast);
\path[line] (manual) |- (bufsoutheast);
\path[line] (environment) -- (visual);
\path[line,latex-latex] (environment) -- (manual);

%\path (patmatch.north) ++(0,0) coordinate (helper3);
\path[line] (buffers) -| (helper2) |- (helper1) -| (patmatch.north);
\path[line] (procmem) -- (patmatch);
\path[line] (patmatch) -- (prodexec);

\path[line] (declarative) |- (buffers.east);

\path (buffers.east) ++(0,+0.3) coordinate (bufnortheast);
\path[line] (prodexec) |- (bufnortheast);

%\path  (prodexec.south)+(0,-1) node (belowprodex) {};
%\path[draw] (prodexec.south)--(belowprodex.north);
%\path 
%\path[draw]  (belowprodex.north) -| (belowdecl);
%(intersection of prodexec.east--prodexec.west and declarative.south east--declarative.north east)+(1,0) ;

%\path[draw] (prodexec.south) -- + (0,-1) -|  (intersection of prodexec.east--prodexec.west and declarative.south east--declarative.north east) |- (buffers) ;
\end{tikzpicture}
...
\begin{tikzpicture}[node distance = 5.5cm, auto]
 ...
 \node[element, left of=goal-chunk] (current-number) {2}; 

    % Draw edges
 \path[slot] (goal-chunk) -- node[above] {current-number} (current-number);
\end{tikzpicture}
...
\tikzstyle{rule}=[rectangle, draw=black,  
        rounded corners, anchor=west, text=black, text width=9cm]

\begin{center}
\begin{tikzpicture}[node distance=0.5cm]
\node (count-rule) [rule, rectangle split, rectangle split parts=5, rectangle split part fill={red!30, yellow!20,yellow!20,red!20,red!20}] {
        \textbf{count-rule}
        \nodepart{two} IF the goal is to count, the current number is $n$
        \nodepart{three}AND the retrieval buffer holds a chunk of type \emph{count-fact} with the \emph{first} value $n$ and the \emph{second} value $m$
        \nodepart{four}THEN set the current number in the goal to $m$
        \nodepart{five}AND send a retrieval request for a chunk that has $m$ in its \emph{first} slot
    };
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}[node distance = 4cm, auto]
 ...
 \node[element, below of=addition-fact, node distance=2cm] (sum) {7}; 
    ...
    % Draw edges
  \path[slot] (addition-fact) -- node[above] {arg\textsubscript{1}} node[below] {$S_{ji}$} (arg1);
  \path[slot] (addition-fact) -- node[above] {arg\textsubscript{2}} node[below] {$S_{ji}$} (arg2);
...
\begin{figure}[htb]
\centering
\tikzstyle{chunk} = [ellipse, draw, fill=blue!20, text centered, text width=2.5em, minimum height=0.8cm]
\tikzstyle{empty} = [ellipse, text centered, text width=2.5em, minimum height=0.8cm]
\tikzstyle{line} = [draw,diamond-latex]

\begin{tikzpicture}
\matrix[row sep=2mm,column sep=2cm]{
\node (fireman) {fireman}; &
\node[chunk] (a) {1 -- 1};&
\node (bank) {bank};\\
& \node[chunk] (b) {3 -- 1};
...
\node[below of=d, node distance=9mm] (bi) {$B_i$};

\path[draw] (fan) -- (fanlabel);
\path[draw] (bi) -- (d);
\end{tikzpicture}
\caption{The ellipses represent chunks which encode one fact like ``a hippie is in the park'' (red highlighted chunk $i$). The words are primitive elements. The special arrows show the direction of activation spreading from an element to a chunk. The numbers in the chunks signify the fan of the associated element, e.g. the fan of the element \emph{hippie} is 3. The associative strength of the element hippie to the highlighted chunk $i$ is $S_{\mathrm{hippie},i} = S - \mathrm{ln}(\mathrm{fan_{hippie}}) = S - \mathrm{ln}(3)$. \cite[fig.~6]{anderson_integrated_2004}}
\label{fig:fans}
\end{figure}
...
\begin{figure}[htb]
\centering
\tikzstyle{chunk} = [ellipse, fill=blue!20, draw, text centered, text width=3.1em, minimum height=1cm]
\tikzstyle{source} = [rectangle, fill=blue!20, draw, text centered, text width=4em, minimum height=1cm]
...
\matrix[column sep=2cm,row sep=2mm] {
&\node[source] (source1) {source\textsubscript{1}}; 
& &
\node[chunk] (c1) {chunk\textsubscript{1}};\\
\node[buffer] (goal) {
\begin{tikzpicture}
\node (bufname) {\textbf{goal}};
\node[below of=bufname] (slot1) {slot\textsubscript{1}};
\node[below of=slot1] (slot2) {slot\textsubscript{2}};
\end{tikzpicture}

}; & & &
\node[chunk] (c2) {chunk\textsubscript{2}};\\\
&
\node[source] (source2) {source\textsubscript{2}};
& & \node[chunk] (c3) {chunk\textsubscript{3}};
\\
};
