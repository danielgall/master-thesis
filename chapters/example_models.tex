...
I.e., the duplicate slot test for the slot \lstinline|value| is reduced to one \lstinline|chunk_has_slot(B,value,E)| head constraint and a guard, which states that \lstinline|=obj2| $\neq$ \lstinline|=cat|: \lstinline|E \== D|. After this small change, the model can be run with various queries, i.e. goal chunks as described in \cite[unit 1, pp. 24\psqq]{actr_tutorial} and appendix~\ref{app:ex:semantic_model}.
