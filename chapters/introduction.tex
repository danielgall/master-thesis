...
\begin{tikzpicture}[node distance = 2cm, auto]
    ...
    \node [block, below of=experiment, node distance=3cm] (cogarch) {cognitive architecture};
 \node [cloud, right of=experiment, node distance=5cm] (assumptions) {\parbox{3cm}{\centering assumptions about a particular domain}};
  \node [block, below right of=cogarch, node distance=3cm] (cogmod) {cognitive model};

    % Draw edges
 \path [line] (experiment) -- (cogarch);
