...
The straight-forward definition of a data type for chunk specifications is just to use the specification like in definition~\ref{def:abstract_methods_chunk_store}: Since the term \lstinline[language=]|(name isa type slot_1 val_1 ... slot_n val_n)| is just a list in Lisp and specifies a chunk uniquely, a similar Prolog term can be used:
...
\node[below of=v1, node distance=9mm] {\parbox{2cm}{\centering \rotatebox{270}{$\in$} \\\vspace{1mm}$C \cup E$}};
\node[below of=v2, node distance=9mm] {\parbox{2cm}{\centering \rotatebox{270}{$\in$} \\\vspace{1mm}$C \cup E$}};
&
 \node {$\}$};
&
\node (nil) {$\mathtt{nil}$}; 
...
\path[line] (c) -- node[left] {$HasSlot$} (v1);
...
\begin{tikzpicture}[node distance = 3cm, auto]
 \node[event] (e1) {\parbox{1cm}{\centering $e_1$\\ $t_1$}}; 
 \node[event, right of=e1] (e2) {\parbox{1cm}{\centering $e_2$\\ $t_2$}}; 
 \node[event, right of=e2] (e3) {\parbox{1cm}{\centering $e_3$\\ $t_3$}}; 
 \node[right of=e3] (dots) {\dots}; 
\draw let 
  \p1 = (e1.west),
 \p2 = (dots.east)
in
node[node distance=2cm, minimum width=\x2-\x1] at ({(\x2+\x1)/2},\y1-15mm) {$t_1 \le t_2 \le t_3 \le \dots$};
