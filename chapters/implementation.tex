...
\begin{description}
 ...
 \item[Identifiers] Everything that starts with a letter and contains only letters or numbers. An identifier ends, if a special character or a white-space is read from the input. An exception is the \lstinline|-| character, which can be used within identifiers. \emph{Example:} \lstinline|count, addition-fact, chunk1|
...
extend figure illustrating overall process with details

Idea: finite automaton, neutral state: start. States: identifier, number, image of automaton?

\subsubsection{Parser}

uses dcg rules, just implements a simplified version of the grammar presented above, produces a parse tree (explain how this is done with dcg rules), extend last figure illustrating overall process with details

\subsubsection{Translation Component}

writer as shown by Amira Zaki in the lecture

\subsection{Problems with the current implementation}

only subset of the grammar
problems with duplicate slot tests
no error handling, no error messages
translation component is a little bit ugly -> new CHR based version is in development but not finished yet
