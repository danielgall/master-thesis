...
On top of the rules in listing~\ref{lst:add_chunk_rules}, there must be added a rule that deletes a primitive element (i.e. a chunk of type \verb|chunk|), if the user introduces a real chunk with the name of this element:
...
However, since the match set is distributed over a set of \verb|match_set| constraints, it would be desirable to collect all those matches in one set. This can be triggered by the constraint \verb|collect_matches/1|, which returns the complete match set in its arguments as soon as there is only one \verb|match_set| left:
...
In the second rule, if no two match sets are in the store, the result of the \verb|collect_matches| operation is the match set. The same applies for the last rule, where no match set is in the store and therefore the result is empty. Note, that this implies, that the rules have to be applied from top to bottom, left to right\footnote{This is called the refined operational semantics of CHR}.
...
\verb|q(T1,P1,E1)| $\prec$ \verb|q(T2,P2,E2)|, i.e. the object \verb|q(T1,P1,E1)| is the predecessor of the object \verb|q(T2,P2,E2)|, if \verb|T1| $<$ \verb|T2|. In the case that \verb|T1 = T2|, then \verb|q(T1,P1,E1)| $\prec$ \verb|q(T2,P2,E2)| if \verb|P1| $>$ \verb|P2|. So, events with smaller times will be returned first. If two events appear at the same time, it is possible to define a priority and the event with the higher priority will be returned first. 
\end{definition}
...
where \verb|conflict_resolution/0| just schedules the next \verb|fire| event and is defined as:
...
Lisp functions are lists (indicated by round braces \verb|(| and \verb|)|), which have the functor as their first element and the function arguments in the rest of the list. Hence, each Lisp function \verb|(f arg1 arg2 ...)| called in the model definition of an ACT-R model is translated into the adding of a CHR constraint \verb|lisp_f([arg1, arg2, ...])|. For all used functions, there has to be a default implementation in the module \verb|std_lisp|. In this implementation, the concept is reduced to simple procedural calls ignoring return values, which is adequate for most of the pure ACT-R models without the experiment environment. Here is an example implementation of the lisp function \verb|(chunk-type name slot1 slot2 ...)|:
...
In ACT-R, configuration variables which control the behaviour of the architecture are set with the Lisp function \verb|(sgp :Var Val)|, which sets the variable \verb|Var| to \verb|Val|. To handle such configuration variables, a configuration module can be added to the CHR implementation, that offers an interface to set variables and to ask for their values:
...
As soon as the intermediate result has been calculated, two \verb|base_level_part| constraints can be merged, by adding their \verb|B| values up according to the base-level learning equation~\eqref{eq:base_level_learning}:
...
If \verb|J| does not appear in the slots of chunk \verb|I|, then $S_{ji} := 0$ by definition.
...
The last rule will only be applied, if both of the other rules did not match. This is the case, if there has no \verb|max| constraint been put to the store, which does only occur, if no chunk matched the request. This also leads to an empty chunk as a result.
...
ACT-R offers some configuration variables which influence the retrieval process. In the CHR implementation, some of those configuration variables are implemented in the system according to the configuration infrastructure as described in section~\ref{configuration}. In ACT-R, configuration variables are set with the command \verb|(sgp :Var Val)| which sets the value of the variable \verb|Var| to \verb|Val|. 

\begin{description}
 \item[Turn on the subsymbolic layer] The variable \verb|esc| that can be set to \verb|t| for \emph{true} and \verb|nil| for \emph{false} controls the activation of the subsymbolic layer for retrieval requests. In CHR, the declarative module observes the value of this variable by the observer interface presented in section~\ref{configuration}. Depending on the value of the variable, the constraint \verb|subsymbolic/0| is present or not:
...
For each rule only involved in the subsymbolic layer, the constraint \verb|subsymbolic/0| is added to its kept head, so the rule only fires if the subsymbolic layer is turned on.

\item[Retrieval threshold] The retrieval threshold $\theta$ is set by the configuration variable \verb|rt|.
\item[Latency factor] The latency factor $F$ is set by the configuration variable \verb|lf|.
...
\item[Decay parameter] The decay parameter $d$ is set by the configuration variable \verb|bll|.
...
The first rule adds the matching rule to a conflict set without computing anything, the second rule actually performs the calculations as soon as the \verb|apply_rule| constraint is present.
...
As soon as the \verb|fire| constraint is present (so the recognize cycle/conflict resolution process begins), each matching rule adds a \verb|conflict_set/1| constraint with its name. The last rule finishes the recognize cycle by deleting the fire constraint \emph{after} all rules that match had their chance to add a \verb|conflict_set| constraint and adds an empty \verb|conflict_set| constraint, indicated by \verb|[]|. The constraints store contains at the end of this phase a bunch of \verb|conflict_set| constraints that represent the matching rules plus an empty \verb|conflict_set| constraint. If no rule matches, there is only an empty \verb|conflict_set| constraint in the store.

The last rule also triggers the choosing process by adding the constraint \verb|choose|. The following rules handle the choosing process:
...
The first rule deletes the empty \verb|conflict_set| constraint, if there are other \verb|conflict_set| constraints present, i.e. there has been a rule that can fire.

The second rule assumes, that for each production rule \verb|p| in the procedural memory there is a \verb|production_utility(p,u)| constraint that holds the utility value \verb|u| of the production \verb|p|. If there are two \verb|conflict_set| constraints in the store, the one with the higher utility value will be kept and the other removed from the store.

If the rules have been applied to exhaustion, there is only one \verb|conflict_set| constraint in the store -- either an empty one ore one with the name of a rule. The following rules handle the choosing process:
...
The first rule is only applicable, if there were no matching rules and the empty \verb|conflict_set| is still present. Then this fact is indicated by a \verb|no_rule| constraint.

In the second rule, the firing of the last remaining rule is scheduled 50\,ms from the current time, as it is described in section~\ref{procedural_knowledge}. The event is the \verb|apply_rule(P)| constraint, which leads the second rule in listing~\ref{lst:conflict_resolution_scheme} to fire, which performs the actions of the rule.

When the chosen rule is applied, its actions are performed and eventually, a \verb|conflict_resolution| constraint is added to the store. This constraint leads the next conflict resolution event to being scheduled:
...
If there was no matching rule, the \verb|no_rule| constraint is in the store. This leads the next conflict resolution event to be scheduled after the next event (which may lead to a change of the system state):
...
The reward a rule can distribute, can be saved in a \verb|reward/2| constraint. If a rule is applied, the time of application can be saved in a \verb|to_reward/2| constraint, that states that the rule in the constraint has been applied and therefore receives a part of the next reward as soon as it occurs: 

\begin{lstlisting}
<<apply_rule(P)>> ==> <<<P \== []>>> | get_now(Now), to_reward(P,Now).
\end{lstlisting}

Note, that the \verb|apply_rule/1| constraint from the last section is used to determine, when a rule is fired.
...
This will reward all rules that have a \verb|to_reward/2| constraint in the store, which leads to a new production utility value:
...
In the end, there has to be a rule that cleans up the reward trigger, after all the rules have been rewarded. This ensures, that the next \verb|to_reward| constraints are not immediately consumed by the last reward trigger:
...
There are some methods to influence the conflict resolution process using the ACT-R command \verb|(spp ...)| that can set values for individual production rules. This command is translated into CHR as described in section~\ref{lisp_functions}. Additionally, there are some configuration variables that can be set -- as described in section~\ref{configuration} -- by the command \verb|(sgp ...)|:

\begin{description}
 \item[Setting the utility of a rule] Utilities can be set statically by the user by the ACT-R command \verb|(spp P :u U)|, which sets the utility of the rule \verb|P| to \verb|U|.
 \item[Setting the reward of a rule] The amount of reward a rule can distribute is set by the user. By default, no rules have a reward to trigger. With the ACT-R command \verb|(spp P :reward R)|, the reward of the rule \verb|P| is set to \verb|R|. This command is also implemented among the standard lisp methods.  
 \item[Default values and turning the utility mechanisms off] At the moment, there is no command in the CHR implementation that allows to turn off the conflict resolution mechanisms: The system sets a default utility value for each production rule and no rewards in the initialization process (the rule \verb|init @ run <=> ...| as described in section~\ref{initialization}). The user is able to set other initial utility values and rewards for particular rules. If no rewards are set, the utility values will remain at their initial values. Hence, if the user does not set any utilities or rewards, each rule will have the same utility value that does not change. If there are competing rules, the conflict resolution process will pick one. This can be regarded as turned off utility mechanisms.
 \item[Learning rate] The learning rate $\alpha$ is set by the command \verb|(sgp :alpha A)| that sets the configuration variable \verb|alpha| to \verb|A|.
\end{description}
