...
Additionally, from the definition of a chunk store it is known, that the $HasSlot$ and the $Isa$ relations have to be left-complete. \FIXME{correct expression! correct definitions!} Therefore, for every chunk \lstinline|c| in the store, exactly one \lstinline|isa(c,t)| constraint has to be in the store. For each \lstinline|chunk_type_has_slot(t,s)| constraint, a \lstinline|chunk_has_slot(c,s,v)| constraint has to be defined. If one wants to express, that a chunk has an empty slot, he might use \lstinline|nil| for the value to indicate that. Note that \lstinline|nil| must not be a chunk name or chunk-type name.
