...
&\fun{slots}(c) = \{ \enspace (s,v) \enspace | \enspace (c,s,v) \in HasSlot \enspace \} \enspace \text{and}\\
...
%[caption={rules for \texttt{add\_chunk\_type}}]
\begin{lstlisting}
<<add_chunk_type(CT, [])>> <=> 
  chunk_type(CT).
...
%[caption={rules for \texttt{return\_chunk}}]
\begin{lstlisting}
...
\path[line] (b1) -- node[left] {$\rel{Holds}$} (c);
\path[line] (b2) -- node[right] {$\rel{Holds}$} (nil);
\path[line] (c) -- node[left] {$\rel{Isa}$} (t);
\path[line] (c) -- node[left, near end] {$\rel{HasSlot}$} (v1);
\path[line] (c) -- node {$s_1$} (v1);
\path[line] (c) -- node[right] {$\rel{HasSlot}$} (v2);
\path[line] (c) -- node[left] {$s_2$} (v2);
\end{tikzpicture}
\caption{A buffer system $(B,\Sigma,\rel{Holds})$ with two buffers $b_1, b_2 \in B$ and a chunk store $\Sigma = (C, E, \mathcal{T}, \rel{HasSlot}, \rel{Isa})$. The relations $\rel{Holds}$, $\rel{Isa}$ and $\rel{HasSlot}$ are illustrated by arrows. The buffer $b_1$ holds a chunk $c$ of type $T = (t,\{s_1, s_2\})$ and with values $v_1$ and $v_2$. Buffer $b_2$ is empty. The buffer system is clean, since only the chunks which are held by a buffer are in the set of chunks $C$ of the chunk store.}
...
This example introduces the ACT-R syntax to define the left hand side of a production rule. The left hand side of the counting rule specified in the example in section~\ref{example_counting} can be defined as follows:\enlargethispage{-\baselineskip}
...
In this example, the translation of a simple ACT-R production rule to a CHR rule is shown. Hence, the translation of the production rule in example~\ref{ex:counting} is inspected. It leads to the following translation:
...
In the current translation scheme, the order of the rule applications does not match the semantics as described in the ACT-R theory (see section~\ref{procedural_knowledge}). Consider the following rules in a compact notation:
...
In CHR this special case can be handled by adding a guard for each variable occurring in a positive slot-test checking that this variable does not equal \lstinline|nil|. For negated slot tests, this is not the case: The following rule matches also a chunk with an empty \lstinline|spam| slot:
...
\begin{umlcomponent}{model}
  \umlbasiccomponent[x=1.5,y=-4]{user-defined rules}
  \begin{umlcomponent}[]{core}
    \begin{umlcomponent}[x=4]{buffers}
        \umlbasiccomponent{chunk-store}
    \end{umlcomponent}
    \umlbasiccomponent[x=-2,y=1]{stdlisp}
  \end{umlcomponent}
\end{umlcomponent}

\begin{umlcomponent}[x=12, y=1.5]{declarative module}
        \umlbasiccomponent{chunk-store}
\end{umlcomponent}

\umlbasiccomponent[x=1,y=-6.5]{scheduler}
\umlbasiccomponent[x=12, y=-4.5]{configuration}


%\umlrequiredinterface[distance=4cm, padding=12cm]{core}
%\umlprovidedinterface[interface=IScheduler]{scheduler}

 \umlVHassemblyconnector[interface=IScheduler, distance=3cm, with port, anchor1=320, anchor2=east]{core}{scheduler}
 ...
 \umlHVHassemblyconnector[with port, interface=IConfiguration, anchor1=340, anchor2=west]{model}{configuration}

 \umlHVHassemblyconnector[anchor1=10, anchor2=160,with port, interface=IAddDM]{buffers}{declarative module}

 \umlHVHassemblyconnector[arm1=3, anchor1=-10, anchor2=200,with port, interface=IModule]{buffers}{declarative module}

\umlport{core}{240} 
\umlport{stdlisp}{south} 
...
\umlport{user-defined rules}{north}
 \umlVHVassemblyconnector[interface=ICore]{user-defined rules-north-port}{core-240-port}
% 
\umlport{buffers}{210}
 \umlVHassemblyconnector[interface=IBuffers]{core-240-port}{buffers-210-port}
% \umlport{stdlisp}{west}
 \umlVHVassemblyconnector[interface=ILispFunctions]{core-240-port}{stdlisp-south-port}
...
\begin{lstlisting}
% do not add chunks with :new tag, remove identical information
...
The system periodically removes the first event from the queue, sets the current time to the time of the event and executes it which may lead to new events in the queue. The queue organizes the right order of the events automatically. Figure~\ref{fig:scheduler:b} shows the effect of the removal to the queue and the system time. With this approach, the simulation of a parallel execution of ACT-R can be achieved: Each buffer action on the RHS of a rule just schedules an event that actually performs the action at a specified time (the current time plus its duration). The central part of the scheduler is a \emph{priority queue} which manages a list of events and returns them by time. It is described in the next section.
...
% Time1 < Time2 -> event with time1 first, ignore priority
...
\begin{lstlisting}[aboveskip=0.5\baselineskip]
% queue with more than one element
<<de_q(X), s --> A, A --> B>> <=>
  X = A,
  s --> B.
...
which removes the \lstinline|fire| constraint if still present and states that no rule has been fired, because otherwise the firing of a rule would have removed the \lstinline|fire| constraint. If no rule was applicable, a new \lstinline|fire| event is scheduled after the next event:\enlargethispage{-\baselineskip}
...
% just add start_request event with current time
...
<<<delay-name>>> @
  ...
  {buffer tests}>> 
  ==> {guard} | 
  conflict_set(rule(name, {Variables in the head of the rule})).
<<<name>>> @
  <<{buffer tests}, 
  apply_rule(rule(name, {Variables in the head of the rule}))>> 
  <=> {guard} | 
...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=5cm,
                    semithick]
  \tikzstyle{every state}=[fill=yellow!20,draw,text=black,minimum size=1.5cm]
...
getToken(start, [X|Xs],Rest,LT) :-
  letter(X), 
  % find rest of word
  getToken(identifier, Xs,Rest,[X],T), 
...
getToken(identifier,Rest,Rest,S,W) :-
  % tokens are accumulated in wrong order
  reverse(S,S1),
  atom_chars(W,S1). 
\end{lstlisting}
