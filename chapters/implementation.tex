...
So $E$ is now represented by $\{ c \in C | c \enspace Isa \enspace (chunk,\emptyset)$ in the implementation.
...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=5cm,
  ...
  \node[state,accepting]         (identifier) [right of=start]      {identifier};
  \node[state,accepting]         (number)     [below of=start]      {number};
  ...
  \path (start)      edge [loop above] node {break} (start)
        ...
        (identifier) edge [loop above] node {letter, number, \texttt{-}} (identifier)
